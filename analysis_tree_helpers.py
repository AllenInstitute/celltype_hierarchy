    ...
    for node in current_node_list:
        ...
        if len(children_node_list) > 0:
            ...
            if sorted([node.name for node in current_node_list_copy]) not in valid_classes:
                valid_classes = get_valid_classifications(current_node_list_copy,
                                                          C_list=C_list,
                                                          P_list=P_list,
...
class HTree():
    ...
    def __init__(self, htree_df=None, htree_file=None):
        ...
        if htree_file is not None:
            ...
            htree_df = htree_df.rename(
                columns={'label': 'child', 'leaf': 'isleaf'})
            ...
            htree_df = htree_df.sort_values(by=['y', 'x'],
                                            axis=0,
    ...
    def get_all_descendants(self, leafonly=False):
        ...
        for key in np.unique(np.concatenate([self.child, self.parent])):
            descendant_dict[key] = self.get_descendants(
                node=key, leafonly=leafonly)
    ...
    def get_mergeseq(self):
        ...
        ordered_merge_parents = np.setdiff1d(
            self.parent, self.child[self.isleaf])
        y = []
    ...
    def get_subtree(self, node):
        ...
        if len(subtree_node_list) > 1:
            ...
            subtree_df = subtree_df[subtree_df['child'].isin(
                subtree_node_list)]
        else:
    ...
    def update_layout(self):
        ...
        for node in parents:
            descendant_leaf_nodes = self.get_descendants(
                node=node, leafonly=True)
...
def simplify_tree(pruned_subtree, skip_nodes=None):
    ...
    for node in skip_nodes:
        ...
        if node_parent.size != 0:
            ...
            print('Remove {} and link {} to {}'.format(
                node, node_parent, node_child))
            ...
            simple_tree_df.drop(simple_tree_df[(simple_tree_df.child == node) | (
                simple_tree_df.parent == node)].index, inplace=True)
